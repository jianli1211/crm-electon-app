...
import { PositionLabelItem } from "./position-label-item";
...
export const PositionLabelsDialog = (props) => {
  const {
    open,
    onClose,
    teams,
  ...
  const getLabels = async () => {
    try {
      ...
      const res = await riskApi.getPositionLabels(params);
  ...
  const onLabelSubmit = async (data) => {
    try {
      setIsLoading(true);
      const result = await riskApi.createPositionLabel(data);

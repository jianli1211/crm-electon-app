...
import { ArticleLabelItem } from "src/components/article-label-item";
...
export const ArticleLabelsDialog = ({ open, onClose, teams, getLabelList, title = "Customer labels" }) => {
  const {
  ...
  const getLabels = async () => {
    try {
      ...
      const res = await blogApi.getArticleLabels(params);
  ...
  const onLabelSubmit = async (data) => {
    try {
      setIsLoading(true);
      const result = await blogApi.createArticleLabel(data);
      const newData = [...labelData];
      newData.push(result?.label);
      setLabelData(newData);
      toast.success("Article label successfully created!");
      setIsLoading(false);
      setTimeout(() => getLabelList(), 1500);
    } catch (error) {
      setIsLoading(false);
      console.error("error: ", error);
    }

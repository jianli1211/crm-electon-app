  ...
  return (
    ...
    >
      <ChatThreadToolbar
        ...
        calling={false}
        isClientChat
        callConversationId={props?.customer?.client?.call_conversation_id}
      />
      <Divider />
      <Box
        sx={{
      ...
      <ChatMessageAdd
        ...
        ticket={ticket}
        conversationId={threadKey}
        sendMessageAccess={(currentMember?.acc_send_message || currentMember?.owner || ticket?.default) ?? false}
        sendMediaAccess={(currentMember?.acc_send_media || currentMember?.owner || ticket?.default) ?? false}
        onParticipantsGet={handleParticipantsGet}
        brandId={props?.customer?.client?.internal_brand_id}
      />
    </Stack>
  );
};

  ...
  return (
    <ClickAwayListener onClickAway={onClickAway}>
      <Box
        ...
        {showTip && (
          <Box sx={{ py: 2 }}>
            <Tip message="Enter a ticket subject" />
          </Box>
        )}
        {showResults && (
          <>
                ...
                <Box sx={{ py: 2 }}>
                  ...
                  <List>
                    {results.map(({ ticket }) => (
                      <ListItemButton
                        key={ticket.id}
                        onClick={() => handleSelect(ticket.conversation_id)}
                      >
                        <ListItemAvatar>
                          <Avatar
                            src={ticket.conversation.avatar ? ticket.conversation.avatar?.includes('http') ? ticket.conversation.avatar : `${getAPIUrl()}/${ticket.conversation.avatar}` : ""}
                            sx={{
                              height: 32,
                              width: 32
                            }}

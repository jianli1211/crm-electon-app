...
export const SettingSidebar = (props) => {
  ...
  const sideMenuList = [
    ...
    { label: "Company Emails", value: "company_emails" },
    { label: "Global SMTP Settings", value: "global_smtp_settings" },
    { label: "Default Email Signature", value: "email_signature" },
    { label: "Automated Emails", value: "automated_emails" }
  ];
      ...
      >
        ...
        <Stack direction="column" mt={2} gap={1}>
          {sideMenuList.filter(item => {
            if (item.value === "email_templates") {
              return user?.acc?.acc_e_settings_email_templates === undefined || user?.acc?.acc_e_settings_email_templates
            }
            return true;
          }).map((item) => (

...
export const EditSavingAccount = ({
  account,
  open,
  onClose,
  onGetAccounts = () => { },
  ...
  useEffect(() => {
    if (account) {
      ...
      if (account?.settings) {
        const options = JSON.parse(account?.settings);
        setOptions(options);
      } else {
        setOptions([]);
      }
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await clientDashboardApi.updateSavingAccount(account?.id, formData);
      toast.success("Saving account successfully updated!");
      onClose();
      setTimeout(() => {
        onGetAccounts();
        reset();

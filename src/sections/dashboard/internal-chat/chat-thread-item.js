...
export const ChatThreadItem = (props) => {
  ...
  const avatar = useMemo(() => {
    if (thread?.participant_accounts?.length === 2) {
      const avatar = thread?.participant_accounts?.find(
  ...
  const isActive = useMemo(() => {
    if (thread?.participant_accounts?.length === 2) {
      const chatMember = thread?.participant_accounts?.filter(
        (item) => item?.id !== account_id
      );
  ...
  const isDateYesterday = useMemo(
    () =>
      lastActivity
        ? isYesterday(startOfDay(new Date(toLocalTime(lastActivity))))
        : isYesterday(startOfDay(new Date())),
    [lastActivity, user?.timezone]
  );
  ...
  const timeFormat = useMemo(() => {
    const diffInDays = lastActivity
      ? differenceInDays(
          startOfDay(new Date()),
          startOfDay(new Date(toLocalTime(lastActivity)))
        )
      : 0;
        ...
        >
          ...
          <Stack direction="column" spacing={1} alignItems="flex-end">
            <Typography color="text.secondary" variant="caption">
              ...
              {lastActivity
                ? format(new Date(toLocalTime(lastActivity)), timeFormat)
        ...
        >
          ...
          >
            <Typography
              ...
              variant="subtitle2"
            >
              {thread?.last_message_by_id === account_id &&
              !!thread?.last_message
                ? "You:"
                : null}{" "}
              {thread?.last_message ?? ""}
          ...
          {unReadCount > 0 && <Chip size="small" label={unReadCount} />}
        </Stack>
      </Box>
    </Stack>
  );

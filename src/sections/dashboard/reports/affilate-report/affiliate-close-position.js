...
export const AffiliateClosePositions = ({report, isLoading, setCloseTable}) => {
  const theme = useTheme();
  ...
  const generateTableInfo = () => {
    const tableInfo = [
      ['Total Close Positions', 'Open Volume', 'Open Leveraged Volume', 'Open PNL'],
      [...openKeys?.map((item)=> (`${report[item.key]<0?"-":""}${Math.abs(report[item.key])??""}`))],
      ['Top Assets'],
      [...closeTopAssets?.map((item)=> (`${item?.asset_name}: (Volumne: $${report ? item?.asset_volume ?? '0.0' : ''}, ${item?.asset_price?.toFixed(2)}  ${item?.asset_price < 0 ? 'decreased vs last period' : 'increased vs last period'})`))]
    ];
    setCloseTable(tableInfo);
  }
    ...
    <Grid xs={12} xl={6}>
      <Card sx={{ p:2, display:'flex', flexDirection:'column', gap:3, pt:3, height:1 }}>
        ...
        <Grid container spacing={2}>
          <Grid xs={12} sm={7}>
            <Stack>
              {closeTopAssets?.length>0 && <Typography variant="subtitle2" whiteSpace='nowrap' color='text.primary' pb={2}>Top Assets:</Typography>}
              { isLoading?
          ...
          {closeTopAssets?.length > 0 ?
            <Grid xs={12} sm={5} sx={{display:"flex", flexDirection:"row", justifyContent:"center", alignItems:"center"}}>
              <Stack>
                ...
                {closeTopAssets?.length>0 && <Divider/>}

...
export const AffiliateOpenPositions = ({report, isLoading, setOpenTable}) => {
  const theme = useTheme();
  ...
  const openKeys = [
    ...
    { key: "open_leveraged_amount",
      label: "Open Leveraged Volume"
    },
    { key: "open_pnl",
      label: "Open PNL"
    },
  ...
  const openTopAssets = useMemo(()=> {
    if(report) {
      return report?.open_top_assets;
    }
    return undefined;
  }, [report]);
  ...
  const donutInfo = useMemo(()=> {
    if(report) {
      const topOpens = report?.open_top_assets;
  ...
  const generateTableInfo = () => {
    const tableInfo = [
      ['Total Open Positions', 'Open Volume', 'Open Leveraged Volume', 'Open PNL'],
      [...openKeys?.map((item)=> (`${report[item.key]<0?"-":""}${Math.abs(report[item.key])??""}`))],
      ['Top Assets'],
      [...openTopAssets?.map((item)=> (`${item?.asset_name}: (Volumne: $${report ? item?.asset_volume ?? '0.0' : ''}, ${item?.asset_price?.toFixed(2)}  ${item?.asset_price < 0 ? 'decreased vs last period' : 'increased vs last period'})`))]
    ];
    setOpenTable(tableInfo);
  }
        ...
        <Grid container spacing={2}>
          ...
          <Grid xs={12} sm={5} pr={2}>
            <AffiliateStats
              chartSeries={[
                {
                  data: report?.open_position_chart??[]
                }
              ]}
            />
          </Grid>
        </Grid>
        {openTopAssets?.length > 0 ?
          <Grid container spacing={2}>
            <Grid xs={12} sm={7}>
              <Stack>
                {openTopAssets?.length>0 && <Typography variant="subtitle2" whiteSpace='nowrap' color='text.primary' pb={2}>Top Assets:</Typography>}
                  { isLoading?
                    [...new Array(4).keys()].map((item)=> (
                    <Grid container spacing={2} key={item} pb={1.5}>
                      <Grid xs={4}>
                        <Skeleton width='50px' /> 
                      </Grid>
                      <Grid xs={4}>
            ...
            <Grid xs={12} sm={5} sx={{display:"flex", flexDirection:"row", justifyContent:"center", alignItems:"center"}}>
              <Stack>
                ...
                {openTopAssets?.length>0 && <Divider/>}

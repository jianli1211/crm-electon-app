...
export const AgentTotal = ({report, isLoading, setBalanceTable}) => {
  const theme = useTheme();
  ...
  const donutChatSeries = useMemo(()=> {
    if(report) {
      return [report?.total_open_positions ?? 0, report?.total_closed_positions ?? 0, report?.total_pending_positions ?? 0 ];
    }
    return [0, 0, 0];
  }, [report]);
  ...
  const donutPercent = useMemo(()=> {
    if(report) {
      const totalValue = report?.total_open_positions + report?.total_closed_positions + report?.total_pending_positions;
      const open = totalValue > 0 ? (report?.total_open_positions / totalValue * 100)?.toFixed(2) : 0;
      const closed = totalValue > 0 ? (report?.total_closed_positions / totalValue * 100)?.toFixed(2) : 0;
      ...
      <Card sx={{ p:2, display:'flex', flexDirection:'column', gap:3, pt:3, height:1 }}>
        <Grid container spacing={2}>
          ...
          <Chart
            dir="ltr"
            type="radialBar"
            series={[isLoading? 0 : (report?.active_clients??0)]}

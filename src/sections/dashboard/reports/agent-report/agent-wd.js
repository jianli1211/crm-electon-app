...
export const AgentWD = ({ report, isLoading, setWDTable }) => {
  ...
  const barChartSeries = useMemo(()=> {
    ...
    const generateTableInfo = () => {
      const tableInfo = [
        ['Total WD', 'WD', 'Credit Out'],
        [`Amount: ${report?.total_wd_amount ?? ""}, Count: ${report?.wd_count ?? ""}(${totalWdKeys?.map((key, index) => {
          const label = index === 0 ? "Approved" : index === 1 ? "Pending" : "Rejected";
          return (
            `${label} : ${report[key] ?? ""}`
          )
        }).join(', ')}), Percent: ${donutPercent[0]}% of total`, `Amount: ${report?.report?.wd_amount ?? ""}, Count: ${report?.report?.wd_count ?? ""}(${wdKeys?.map((key, index) => {
          const label = index === 0 ? "Approved" : index === 1 ? "Pending" : "Rejected";
          return (
            `${label} : ${report[key] ?? ""}`
          )
        }).join(', ')}), Percent: ${donutPercent[1]}% of total`, `Amount: ${report?.credit_out_amount ?? ""}, Count: ${report?.credit_out_count ?? ""}(${creditKeys?.map((key, index) => {
          const label = index === 0 ? "Approved" : index === 1 ? "Pending" : "Rejected";
          return (
            `${label} : ${report[key] ?? ""}`
          )
        }).join(', ')})`],
        ['AVG Total WD', 'WD', 'AVG Credit Out'],
          ...
          <Grid xs={12} sm={7}>
            <Stack>
                {averageList?.map((item, index) => {
                  const change = report ? report[item?.change] ?? 0 : 0; 
                  return (
                    <Grid container spacing={2} key={index} pb={2} sx={{ alignItems: "center" }}>
                      <Grid xs={4}>

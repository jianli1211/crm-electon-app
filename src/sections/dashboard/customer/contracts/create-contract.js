...
export const CreateIcoContract = ({
  open,
  onClose,
  onGetContracts = () => { },
  customerId,
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await clientDashboardApi.createClientContract(formData);
      toast.success("ICO contract successfully created!");
      onClose();
      setTimeout(() => {
        onGetContracts();
        reset();
  ...
  useEffect(() => {
    ...
    setValue("contract_start_date", format(new Date(), 'yyyy-MM-dd'));
    setValue("contract_end_date", format(new Date(), 'yyyy-MM-dd'));
  }, []);
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth>
      <Stack alignItems="center" sx={{ p: 5 }}>
        <Typography variant="h5">Create ICO Contract</Typography>
      </Stack>

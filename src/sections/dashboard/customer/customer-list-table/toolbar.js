...
export const _CustomerTableToolbar = ({
  ...
  const [selectedIBLevel, setSelectedIBLevel] = useState(1);

  // Load selectedFilterValue from localStorage on component mount
  useEffect(() => {
    const savedFilterValue = localStorage.getItem('customerSelectedFilterValue');
    if (savedFilterValue) {
      try {
        const parsedFilterValue = JSON.parse(savedFilterValue);
        setSelectedFilterValue(parsedFilterValue);
      } catch (error) {
  ...
  const handleIBLevelSelect = useCallback((level) => {
    setSelectedIBLevel(level);
    updateFilters({ ...filters, ib_level: level });
  }, [filters, updateFilters]);

  const handleIBLabelKeyDown = (event, level) => {
    if (event.key === 'Enter' || event.key === ' ') {
      handleIBLevelSelect(level);
    }
  };
          ...
          <Stack direction="row" spacing={1} sx={{ ml: 2 }}>
            {[1, 2, 3, 4, 5].map((level) => (
              <Box
                ...
                type="button"
                tabIndex={0}
                aria-label={`Select IB Level L${level}`}
                onClick={() => handleIBLevelSelect(level)}
                onKeyDown={(e) => handleIBLabelKeyDown(e, level)}
                sx={{
                  px: 2,
                  py: 0.5,
                  borderRadius: 2,
                  border: '1px solid',
                  borderColor: selectedIBLevel === level ? 'primary.main' : 'grey.700',
                  backgroundColor: selectedIBLevel === level ? 'primary.main' : 'transparent',
                  color: selectedIBLevel === level ? 'white' : 'grey.200',
                  fontWeight: selectedIBLevel === level ? 700 : 400,
                  cursor: 'pointer',
                  outline: 'none',
                  transition: 'all 0.2s',
                  '&:focus': {
            ...
            <Box
              sx={{
                ...
                '@keyframes pulse': {
                  ...
                  '100%': {
                    boxShadow: (theme) => `0px 0px 0px 3px ${alpha(theme.palette.success.main, 0.1)}`
                  }
                }
              }}
            >

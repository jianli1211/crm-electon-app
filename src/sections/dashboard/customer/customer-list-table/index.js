import { useCallback, useMemo, useState, useEffect, useRef, Suspense, lazy, memo } from "react";
...
import { BulkUpdateFields } from "../bulk-update-fields";
...
import { CustomerCreateComment } from "../customer-create-comment";
import { CustomerCreateQuickEmail } from "../customer-create-quick-email";
import { CustomerCreateSms } from "../customer-create-sms";
...
import { CustomerAssignFormDialog } from "../customer-assign-form-dialog";
...
export const _CustomerListTable = ({
  ...
  const { profiles, handleProfilesGet } = useCallProfiles();
  const { user } = useAuth();
  ...
  const [updateCustomFieldModalOpen, setEditCustomFieldModalOpen] =
    useState(false);
  ...
  const [quickEmailInfo, setQuickEmailInfo] = useState({
  ...
  const [selectedFilterValue, setSelectedFilterValue] = useState(() => {
    // Initialize selectedFilterValue from localStorage
    const savedFilterValue = localStorage.getItem('customerSelectedFilterValue');
    if (savedFilterValue) {
      try {
        return JSON.parse(savedFilterValue);
      } catch (error) {
        console.error('Error parsing saved filter value:', error);
        localStorage.removeItem('customerSelectedFilterValue');
        return undefined;
      }
    }
    return undefined;
  ...
  const columnSettings = useMemo(() => {
    if (columnSetting) {
      setRule(JSON.parse(columnSetting)?.clientTable ?? []);
  ...
  // Save selectedFilterValue to localStorage whenever it changes
  useEffect(() => {
    if (selectedFilterValue !== undefined) {
      localStorage.setItem('customerSelectedFilterValue', JSON.stringify(selectedFilterValue));
    } else {
      localStorage.removeItem('customerSelectedFilterValue');
    }
  }, [selectedFilterValue]);
                ...
                return {
                  ...
                  render: (row) => {
                    if (field?.field_type === "boolean") {
                      ...
                      const color =
                        setting.find((s) => s?.option?.trim() === val?.trim())
                          ?.color ?? "primary.main";
        ...
        onSetCustomFilters(
          customFilters?.map((field, index) => {
            const renderField = {
              ...
              field_type: predefinedFieldType
                ? predefinedFieldType
                : field?.field_type,
            };
            let returnData = {
              ...
              render: (row) => {
                if (field?.field_type === "boolean") {
                  ...
                  const color =
                    setting.find((s) => s?.option?.trim() === val?.trim())?.color ??
                    "primary.main";
                  ...
                  return (
                    <Stack direction="row" alignItems="center" gap={1}>
                      ...
                      {renderEditButton(
                        { rowId: row.id, fieldId: field?.custom_id },
                        handleOpenEditCustomField
                      )}
                    </Stack>
                  );
                ...
                if (row?.client_fields[field?.custom_id]?.length > 15) {
                  return (
                    <Stack direction="row" alignItems="center" gap={1}>
                      {row?.client_fields[field?.custom_id] && (
                        <Link
                          color="text.primary"
                          component={RouterLink}
                          href={`${paths.dashboard.customers.index}/${row?.id}`}
                        ...
                        >
                          <Tooltip
                            key={index}
                            title={row?.client_fields[field?.custom_id]}
                            className="custom-field-value"
                          >
                            <Typography>
                              {row?.client_fields[field?.custom_id]?.substring(0, 15) + "..." ?? ""}
                            </Typography>
                          </Tooltip>
                        </Link>
                      )}
  ...
  const handleCustomerCall = async (numbers, id, callConversationId) => {
    let currentProfiles = profiles;
    
    if (!currentProfiles || currentProfiles.length === 0) {
      try {
        const response = await settingsApi.getCallProfiles();
        currentProfiles = response?.provider_profiles || [];
      } catch (error) {
    ...
    const defaultProvider = currentProfiles?.find((p) => p.is_default && p.enabled)
      ? currentProfiles?.find((p) => p.is_default && p.enabled)
      : currentProfiles?.filter((p) => p?.enabled)?.[0]
      ? currentProfiles?.filter((p) => p?.enabled)?.[0]
      : { name: "Squaretalk" };
    if (defaultProvider?.provider_type === "twilio") {
  ...
  const handleMakeProviderCall = useCallback(async (provider, numbers) => {
    try {
      ...
      toast(`${provider} profile call has started!`);
    } catch (error) {
    ...
    <>
      ...
      <Box sx={{ position: "relative" }} className="customer_table">
        <StyledInfiniteScroll
          ...
          height={
            hasFilter(currentFilters) || hasCustomFilter(customFilters)
              ? window.innerHeight - 444
              : window.innerHeight - 383
          }
          loader={<LinearProgress color="primary" sx={{ width: 1 }} />}

...
export const CustomerBets = ({ customerId }) => {
  ...
  useEffect(() => {
    const customerBetsPerPage = localStorage.getItem("customerBetsPerPage");

    if (customerBetsPerPage) {
      setPerPage(customerBetsPerPage);
    }
  }, []);
  ...
  const handleRemoveChip = (value, target) => {
    ...
    if (target === "non_ids") {
      updateFilters({ non_ids: undefined });
    }
    if (target === "client_id") {
  ...
  const handleMakeExcelData = async () => {
    ...
    try {
      ...
      if (filters?.non_ids?.length) {
        checkParams.non_ids = [filters?.non_ids];
      }
      if (filters?.client_ids?.length) {
      ...
      for (let page = 1; page <= numPages; page++) {
        ...
        if (filters?.non_ids?.length) {
          newParams.non_ids = [filters?.non_ids];
        }
        if (filters?.client_ids?.length) {
  ...
  useEffect(() => {
    setRule(tableSetting?.customerBetsTable ?? []);
  }, [tableSetting]);
    ...
    if (user?.column_setting) {
      try {
        ...
        const updatedTableSetting = {
          ...currentTableSetting,
          customerBetsSorting: userColumnSetting.customerBetsSorting || {},
          customerBetsPinnedFields: userColumnSetting.customerBetsPinnedFields || [],
        };
  ...
  useEffect(() => {
    if (user?.column_setting) {
      setColumnSorting(JSON.parse(user?.column_setting)?.customerBetsSorting);
    }
  }, [user]);
  ...
  useEffect(() => {
    if (user?.column_setting) {
      const updateSetting = {
        ...tableSetting,
        customerBetsSorting: columnSorting,
      };
  ...
  useEffect(() => {
    if (!tableSetting?.customerBetsTable && defaultColumn?.length > 0) {
      const defaultTableSettings = {
        customerBetsTable: defaultColumn.map((item, index) => ({
          id: item.id,
          enabled: item.enabled,
          order: index,
        })),
        customerBetsSorting: {},
        customerBetsPinnedFields: [],
      };
      
      setTableSetting(defaultTableSettings);
      localStorage.setItem("tableSetting", JSON.stringify(defaultTableSettings));
    }
  }, [tableSetting?.customerBetsTable, defaultColumn]);
        ...
        <Card sx={{ backgroundColor: 'background.paper' }}>
          ...
          >
            <Stack direction='row' alignItems='center' gap={2} pl={1} flexWrap="wrap">
              <Typography variant="subtitle1" fontWeight={700}>Bets</Typography>
            </Stack>
            <Stack 

...
export const MerchantEdit = ({ feeId }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await merchantApi.updateRate(feeId, data);
      toast("Merchant fee successfully created!");
      setIsLoading(false);
      setTimeout(
        () =>
          router.push(
            `${paths.dashboard.paymentAudit.merchant.index}/${
              params?.merchantId
            }?fee=${true}`
          ),
        1000
  ...
  const getMerchantFee = async () => {
    try {
      const res = await merchantApi.getRate(feeId);
      setRate(res?.rate);
    } catch (error) {
      console.error("error: ", error);
    }
  ...
  useEffect(() => {
    setValue("payment_type_id", rate?.audit_payment_type_id);

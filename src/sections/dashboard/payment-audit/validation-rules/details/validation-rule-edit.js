...
export const ValidationRuleEdit = ({ handleChangeRule }) => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const res = await validationRuleApi.updateValidationRule(ruleId, request);
      setRule(res?.rule);
      handleChangeRule(res?.rule);
      setIsLoading(false);
      toast("Validation rule successfully updated!");
    } catch (error) {
      setIsLoading(false);
  ...
  useEffect(() => {
    ...
    setAi(rule?.ai ?? "");
    setCode(rule?.code ?? "");
    setStartDate(new Date(rule?.start_date?.split(".")[0]));
    setStartTime(new Date(rule?.start_date?.split(".")[0]));
    setEndDate(new Date(rule?.end_date?.split(".")[0]));
    setEndTime(new Date(rule?.end_date?.split(".")[0]));
  }, [rule]);

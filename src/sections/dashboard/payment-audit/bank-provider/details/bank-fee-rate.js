...
export const BankFeeRate = () => {
  ...
  const getBankFeeRates = async () => {
    setIsLoading(true);
    let request = {
      page: currentPage + 1,
      per_page: perPage,
      q: query?.length > 0 ? query : null,
      audit_bank_id: params?.providerId,
    };
    try {
      const res = await bankProviderApi.getRates(request);
      setRates(res?.rates);
  ...
  const defaultColumn = [
    ...
    },
    {
      ...
      enabled: true,
      render: (row) =>
        format(new Date(row?.start_date?.split(".")[0]), "yyyy-MM-dd HH:mm:ss"),
    },
    {
      id: "ended_at",
      label: "Ended At",
      enabled: true,
      render: (row) =>
        format(new Date(row?.end_date?.split(".")[0]), "yyyy-MM-dd HH:mm:ss"),
    },
    {
      id: "action",
      label: "Actions",
      enabled: true,
      render: (row) => {
        if (!user?.acc?.acc_e_audit_bank) return null;
        return (
          <Tooltip title="Edit">
            <IconButton
              ...
              onClick={() =>
                router.push(
                  `${paths.dashboard.paymentAudit.bankProvider.index}/${params?.providerId}/edit/${row.id}`
                )
              }
            >

...
const validationSchema = yup.object({
  name: yup.string().required("Bank provider name is a required field"),
});

export const BankProviderTable = () => {
  ...
  const getBankProviders = async () => {
    ...
    try {
      const res = await bankProviderApi.getBanks(request);
      setBankProviders(res?.banks);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await bankProviderApi.createBank(data);
      setTimeout(() => {
        getBankProviders();
      }, 1000);
  ...
  useEffect(() => {
    setRule(tableSetting?.merchantTable ?? []);
  }, []);
  ...
  const defaultColumn = [
    {
      ...
      render: (row) => (
        <Link
          ...
          href={`${paths.dashboard.paymentAudit.bankProvider.index}/${row?.id}`}
          sx={{
            alignItems: "center",
            display: "inline-flex",
          }}
    ...
    <>
      <Stack spacing={3} sx={{ mb: 4 }}>
        <Stack direction="row" justifyContent="space-between">
          ...
          {user?.acc?.acc_e_audit_bank === undefined ||
      ...
      <PayWallLayout>
        <Card>
          ...
          {bankProviders?.length === 0 && !isLoading && <TableNoData />}
          {isLoading && <Divider />}

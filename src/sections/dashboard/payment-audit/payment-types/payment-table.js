...
const validationSchema = yup.object({
  name: yup.string().required("Payment type name is a required field"),
});
...
export const PaymentTable = () => {
  ...
  const getPaymentTypes = async () => {
    ...
    try {
      const res = await paymentTypeApi.getPaymentTypes(request);
      setPaymentTypes(res?.payment_types);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await paymentTypeApi.createPaymentType(data);
      setTimeout(() => {
        getPaymentTypes();
      }, 1000);
  ...
  useEffect(() => {
    setRule(tableSetting?.paymentTable ?? []);
  }, []);
  ...
  const defaultColumn = [
    {
      ...
      render: (row) => (
        <Link
          ...
          href={`${paths.dashboard.paymentAudit.paymentType.index}/${row?.id}`}
          sx={{
            alignItems: "center",
            display: "inline-flex",
          }}
    ...
    <>
      <Stack spacing={3} sx={{ mb: 4 }}>
        <Stack direction="row" justifyContent="space-between">
          ...
          {user?.acc?.acc_e_audit_payment_type === undefined ||
      ...
      <PayWallLayout>
        <Card>
          ...
          {paymentTypes?.length === 0 && !isLoading && <TableNoData />}
          {isLoading && <Divider />}

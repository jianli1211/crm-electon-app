...
export const _FilterChips = ({
  ...
  const resetFilters = () => dispatch(thunks.resetIBsFilter());
  ...
  const dateChipVal = (val, label) => {
    if (val) {
      const formattedTime = val ? toLocalTime(val) : "";
      const newChips = val
        ? [
          {
            displayValue: formattedTime?.toString(),
            value: val,
            label: label,
          },
        ]
  ...
  const activeChip = useMemo(() => {
    const newChips = filters?.is_active_ib?.map((item) => ({
      displayValue: item === "true" ? "Yes" : "No",
      value: item,
      label: "IB Active",
    }));
    return newChips;
  }, [filters?.is_active_ib]);
  ...
  const iBNameChip = useMemo(() => {
    const newChips = filters?.ib_name
      ? [
        {
          displayValue: filters?.ib_name,
          value: filters?.ib_name,
          label: "IB Name",
        },
      ]
      : [];
  ...
  const iBIDChip = useMemo(() => {
    const newChips = filters?.ib_id
      ? [
        {
          displayValue: filters?.ib_id,
          value: filters?.ib_id,
          label: "IB ID",
        },
      ]
  ...
  const iBCodeChip = useMemo(() => {
    const newChips = filters?.ib_code
      ? [
        {
          displayValue: filters?.ib_code,
          value: filters?.ib_code,
          label: "IB Code",
        },
      ]
      : [];
  ...
  const networkSizeChip = useMemo(() => {
    const newChips = filters?.total_network_size
      ? [
        {
          displayValue: filters?.total_network_size,
          value: filters?.total_network_size,
          label: "Min Network Size",
        },
      ]
      : [];
    return newChips;
  }, [filters?.total_network_size]);

  const maxNetworkSizeChip = useMemo(() => {
    const newChips = filters?.lte_total_network_size
      ? [
        {
          displayValue: filters?.lte_total_network_size,
          value: filters?.lte_total_network_size,
          label: "Max Network Size",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_total_network_size]);

  const l1Chip = useMemo(() => {
    const newChips = filters?.l1_referrals_count
      ? [
        {
          displayValue: filters?.l1_referrals_count,
          value: filters?.l1_referrals_count,
          label: "Min L1",
        },
      ]
      : [];
    return newChips;
  }, [filters?.l1_referrals_count]);

  const maxL1Chip = useMemo(() => {
    const newChips = filters?.lte_l1_referrals_count
      ? [
        {
          displayValue: filters?.lte_l1_referrals_count,
          value: filters?.lte_l1_referrals_count,
          label: "Max L1",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_l1_referrals_count]);

  const l2Chip = useMemo(() => {
    const newChips = filters?.l2_referrals_count
      ? [
        {
          displayValue: filters?.l2_referrals_count,
          value: filters?.l2_referrals_count,
          label: "Min L2",
        },
      ]
      : [];
    return newChips;
  }, [filters?.l2_referrals_count]);

  const maxL2Chip = useMemo(() => {
    const newChips = filters?.lte_l2_referrals_count
      ? [
        {
          displayValue: filters?.lte_l2_referrals_count,
          value: filters?.lte_l2_referrals_count,
          label: "Max L2",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_l2_referrals_count]);

  const l3Chip = useMemo(() => {
    const newChips = filters?.l3_referrals_count
      ? [
        {
          displayValue: filters?.l3_referrals_count,
          value: filters?.l3_referrals_count,
          label: "Min L3",
        },
      ]
      : [];
    return newChips;
  }, [filters?.l3_referrals_count]);

  const maxL3Chip = useMemo(() => {
    const newChips = filters?.lte_l3_referrals_count
      ? [
        {
          displayValue: filters?.lte_l3_referrals_count,
          value: filters?.lte_l3_referrals_count,
          label: "Max L3",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_l3_referrals_count]);

  const l4Chip = useMemo(() => {
    const newChips = filters?.l4_referrals_count
      ? [
        {
          displayValue: filters?.l4_referrals_count,
          value: filters?.l4_referrals_count,
          label: "Min L4",
        },
      ]
      : [];
    return newChips;
  }, [filters?.l4_referrals_count]);

  const maxL4Chip = useMemo(() => {
    const newChips = filters?.lte_l4_referrals_count
      ? [
        {
          displayValue: filters?.lte_l4_referrals_count,
          value: filters?.lte_l4_referrals_count,
          label: "Max L4",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_l4_referrals_count]);

  const l5Chip = useMemo(() => {
    const newChips = filters?.l5_referrals_count
      ? [
        {
          displayValue: filters?.l5_referrals_count,
          value: filters?.l5_referrals_count,
          label: "Min L5",
        },
      ]
      : [];
    return newChips;
  }, [filters?.l5_referrals_count]);

  const maxL5Chip = useMemo(() => {
    const newChips = filters?.lte_l5_referrals_count
      ? [
        {
          displayValue: filters?.lte_l5_referrals_count,
          value: filters?.lte_l5_referrals_count,
          label: "Max L5",
        },
      ]
      : [];
    return newChips;
  }, [filters?.lte_l5_referrals_count]);
  ...
  const chipList = useMemo(() => ([
    ...
    (activeChip?.length > 0 ?
      {
        ...
        remove: (value) => {
          onDeselectAll();
          const newValue = [...filters?.is_active_ib].filter((item) => item !== value);
          updateFilters({ is_active_ib: newValue });
        }
      } : null
    ),
    (deskChip?.length > 0 ?
    ...
    (networkSizeChip?.length > 0 ?
      {
        chip: networkSizeChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ total_network_size: "" });
        }
      } : null
    ),
    (maxNetworkSizeChip?.length > 0 ?
      {
        chip: maxNetworkSizeChip,
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_total_network_size: "" });
        }
      } : null
    ),
    (l1Chip?.length > 0 ?
      {
        ...
        remove: () => {
          onDeselectAll();
          updateFilters({ l1_referrals_count: "" });
        }
      } : null
    ),
    (maxL1Chip?.length > 0 ?
      {
        ...
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_l1_referrals_count: "" });
        }
      } : null
    ),
    (l2Chip?.length > 0 ?
      {
        ...
        remove: () => {
          onDeselectAll();
          updateFilters({ l2_referrals_count: "" });
        }
      } : null
    ),
    (maxL2Chip?.length > 0 ?
      {
        ...
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_l2_referrals_count: "" });
        }
      } : null
    ),
    (l3Chip?.length > 0 ?
      {
        ...
        remove: () => {
          onDeselectAll();
          updateFilters({ l3_referrals_count: "" });
        }
      } : null
    ),
    (maxL3Chip?.length > 0 ?
      {
        ...
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_l3_referrals_count: "" });
        }
      } : null
    ),
    (l4Chip?.length > 0 ?
      {
        ...
        remove: () => {
          onDeselectAll();
          updateFilters({ l4_referrals_count: "" });
        }
      } : null
    ),
    (maxL4Chip?.length > 0 ?
      {
        ...
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_l4_referrals_count: "" });
        }
      } : null
    ),
    (l5Chip?.length > 0 ?
      {
        ...
        remove: () => {
          onDeselectAll();
          updateFilters({ l5_referrals_count: "" });
        }
      } : null
    ),
    (maxL5Chip?.length > 0 ?
      {
        ...
        remove: () => {
          onDeselectAll();
          updateFilters({ lte_l5_referrals_count: "" });
        }
      } : null
    ),

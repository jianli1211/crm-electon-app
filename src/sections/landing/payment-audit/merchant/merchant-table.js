...
import { merchantMockedList } from "src/utils/constant/mock-data";

export const LandingMerchantTable = () => {
    ...
    <>
      <Stack spacing={3} sx={{ mb: 4 }}>
        <Stack direction="row" justifyContent="space-between">
          <Typography variant="h4">Merchant Fees & Rates</Typography>
          <Button
      ...
      <Card>
        ...
        <Scrollbar>
          <Table sx={{ minWidth: 700 }}>
            ...
            <TableBody>
              {(
                merchantMockedList?.slice(currentPage * perPage, (currentPage * perPage) + perPage)?.map((merchant) => (
                  <TableRow key={merchant?.id} sx={{ whiteSpace: "nowrap" }}>
                    {defaultColumn
                      ?.map((column, index) => (
                        <TableCell key={merchant.id + index}>
                          {column?.render
        ...
        <TablePagination
          ...
          count={merchantMockedList?.length ?? 0}

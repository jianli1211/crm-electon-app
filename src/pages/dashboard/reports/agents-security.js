...
import AgentsSecurityDashboard from "src/sections/dashboard/reports/agents-security";
...
const Page = () => {
  ...
  useEffect(() => {
    if (user?.acc?.acc_v_reports_agents_security === false) {
  ...
  return (
    <>
      <Seo title={`Dashboard: System-Wide Agents Security`} />
        ...
        <PayWallLayout>
          <Container maxWidth="xxl">
            <Stack sx={{ flexDirection: { xs: "column", md: "row" }, gap: { xs: 1 }, mb: { xs: 1, md: 0 }, justifyContent: { xs: "flex-start", md: "space-between" } }}>
              <Typography variant="h4">System-Wide {t(tokens.nav.agentsSecurity)}</Typography>
              <RadioGroup
                row
                name="report-type"
            ...
            <AgentsSecurityDashboard endPoint={endPoint} />
          </Container>
        </PayWallLayout>
      </Box>
    </>

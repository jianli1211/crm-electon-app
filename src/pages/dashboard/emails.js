...
import { ChatSidebar } from "src/sections/dashboard/emails/chat-sidebar";
import { ChatThread } from "src/sections/dashboard/emails/chat-thread";
...
const useEmails = () => {
  ...
  const handleEmailsGet = async () => {
    try {
      const { conversations } = await chatApi.getEmails();
      setEmails(conversations);
    } catch (error) {
...
const Page = () => {
  ...
  const { emails: chats, handleEmailsGet } = useEmails();
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (user?.acc?.acc_v_emails === false) {

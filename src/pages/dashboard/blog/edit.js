...
const Page = () => {
  ...
  useEffect(() => {
    const handleArticleGet = async () => {
      const response = await blogApi.getArticle(params?.articleId, company?.id);
      const {
        question: { question },
      } = response;

      if (question?.banner_url) setCover(question.banner_url);
      if (question?.article_labels?.length) {
        setValue(
          "label_ids",
          question.article_labels.map((l) => l.id)
        );
      }
      setValue("title", question?.name ?? "");
      setValue("seo_title", question?.seo_title ?? "");
      setValue("seo_description", question?.seo_description ?? "");
      setAiEnabled(question?.ai_enabled);
      if (question?.description) setValue("content", question.description);
    };
  ...
  const onSubmit = async (data) => {
    ...
    const updateArticleWithRetry = async (
      formDataToSend,
      maxRetries = 3,
      delay = 2000
    ) => {
      ...
      while (!success && retries < maxRetries) {
        try {
          response = await blogApi.updateArticle(
            params?.articleId,
            formDataToSend
          );
          ...
          retries++;
          console.error(`Article update attempt ${retries} failed:`, error);

          if (retries >= maxRetries) {
            toast.error(
              "Failed to update article after multiple attempts. Please try again."
            );
            setIsLoading(false);
            return null;
          }
    ...
    const response = await updateArticleWithRetry(formData);
